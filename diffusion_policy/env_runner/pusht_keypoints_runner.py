...
class PushTKeypointsRunner(BaseLowdimRunner):
    ...
    def run(self, policy: BaseLowdimPolicy, video_dir_name=None):
        ...
        if video_dir_name is not None:
            media_root = pathlib.Path(self.output_dir).joinpath('media', video_dir_name)
            for i in range(n_inits):
                video_path = all_video_paths[i]
                if video_path is None:
                    continue
                prefix = self.env_prefixs[i].rstrip('/')
                target_dir = media_root.joinpath(prefix)
                target_dir.mkdir(parents=True, exist_ok=True)
                source_path = pathlib.Path(video_path)
                target_path = target_dir.joinpath(source_path.name)
                if target_path.exists():
                    target_path.unlink()
                shutil.move(str(source_path), str(target_path))
                all_video_paths[i] = str(target_path)
